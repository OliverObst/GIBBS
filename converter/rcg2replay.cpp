                         ...
                         const rcsc::rcg::ShowInfoT & show )
{
    ...
    // ball
    M_os << "b " << std::setprecision(3)
         << round(100.0 * show.ball_.x_) / 100.0 << ' ' << round(100.0 * show.ball_.y_) / 100.0 << ' '
         << round(100.0 * show.ball_.vx_) / 100.0 << ' ' << round(100.0 * show.ball_.vy_) / 100.0 << '\n';
...
TextPrinter::printBall( std::ostream & os,
                        const rcsc::rcg::pos_t & ball ) const
{
  os << "b " << std::setprecision(3)
     << round(100.0 * rcsc::rcg::nstohd( ball.x )) / 100.0 << " "
     << round(100.0 * rcsc::rcg::nstohd( ball.y )) / 100.0 << "\n";
...
{
  os << "b " << std::setprecision(3)
     << round(100.0 * rcsc::rcg::nltohd( ball.x )) / 100.0 << " "
     << round(100.0 * rcsc::rcg::nltohd( ball.y )) / 100.0 << " "
     << round(100.0 * rcsc::rcg::nltohd( ball.deltax )) / 100.0 << " "
     << round(100.0 * rcsc::rcg::nltohd( ball.deltay )) / 100.0 << "\n";
                          ...
                          const rcsc::rcg::pos_t & player ) const
{
    ...
    os << " " << std::setprecision(3)
       << round(100.0 * rcsc::rcg::nstohd( player.x )) / 100.0 << " "
       << round(100.0 * rcsc::rcg::nstohd( player.y )) / 100.0 << " "
       << round(100.0 * static_cast< double >( static_cast< short >( ntohs( player.angle ) ) )) / 100.0;
                          ...
                          const rcsc::rcg::player_t & player ) const
{
    ...
    body_deg = round(body_deg * 100.0)/100.0;
    head_deg = round(head_deg * 100.0)/100.0;
    ...
    os << " " << std::setprecision(3)
       << round(100.0 * rcsc::rcg::nltohd( player.x )) / 100.0 << " "
       << round(100.0 * rcsc::rcg::nltohd( player.y )) / 100.0 << " "
      //       << rcsc::rcg::nltohd( player.deltax ) << "x"
